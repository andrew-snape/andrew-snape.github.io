...
Every device in this house that talks to a phone talks to an iPhone. Apple TVs for video, HomePods for audio, iPads on the wall for control -- so the moment I started adding smart devices, the only sane goal was getting every one of them into the Home app rather than juggling a separate app per brand. [Homebridge](https://homebridge.io) is what makes that possible for the devices that don't speak HomeKit natively, and it's been running on the NAS for a while now, quietly bridging things Apple never built official support for.
